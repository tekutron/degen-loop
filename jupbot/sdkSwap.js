// Raydium SDK-based swap (bypasses Trade API transaction build)
// Supports CLMM, CPMM, and Raydium AMM v4 (Liquidity v4 / legacy AMM)
...
async function fetchAmmV4PoolKeys(connection, poolId) {
  ...
  // This object is intentionally "json-ish"; makeAMMSwapInstruction will run jsonInfo2PoolKeys.
  const poolKeys = {
    id: poolPk.toBase58(),
...
async function runSdkSwap({
  ...
  // NOTE: route can be volatile. Re-quote each attempt and rebuild.
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (route.length !== 1) {
      throw new Error(`only single-hop routes supported in sdkSwap.js right now (got ${route.length})`);
    }

    const hop = route[0];
    const poolId = hop.poolId;

    // If AMM v4 pool: build a one-shot Transaction with WSOL wrap + ATA + swap + close.
    ...
    if (poolOwnerInfo.owner.equals(ALL_PROGRAM_ID.AMM_V4)) {
      ...
      // Determine which side is A/B for this swap.
      const tokenAccountInMint = inMint;
      const tokenAccountOutMint = outMint;

      if (!tokenAccountInMint.equals(mintA) && !tokenAccountInMint.equals(mintB)) {
        throw new Error(`input mint ${inMint.toBase58()} not in pool mints ${mintA.toBase58()} / ${mintB.toBase58()}`);
      }
      if (!tokenAccountOutMint.equals(mintA) && !tokenAccountOutMint.equals(mintB)) {
        throw new Error(`output mint ${outMint.toBase58()} not in pool mints ${mintA.toBase58()} / ${mintB.toBase58()}`);
      }

      // WSOL wrap for SOL input.
      // (Raydium SDK helper creates a seeded temp WSOL account and closes it at end.)
      const useWsolIn = inputMint === WSOL;
      if (!useWsolIn) {
      ...
      // compute budget (raydium-sdk-v2 was patched in node_modules to prefer Raydium auto-fee)
    ...
    // Otherwise, try CLMM first; if not found, try CPMM.
    ...
    if (!poolInfo) {
      try {
        ...
        poolInfo = r; // cpmm methods accept this structure
    ...
    const slippage = slippageBps / 10000; // SDK methods accept decimal
